# 1. はじめに
---
本書は、AI-G に最適化されたニューラルネットワークモデルのコレクションを提供します。第 2.1 章で紹介するリポジトリには、分類や物体検出などのタスク向けのさまざまなモデルが含まれています。このリポジトリは評価ボードですぐに実行できる形式で提供され、AI-G の性能と効率を示すベンチマーク結果を含んでいます。
<br/><br/><br/>

## 1.1 概要
---
次の表は、Telechips ハードウェアでサポートされるニューラルネットワークモデルをまとめたものです。各モデルの名称は、性能指標と導入手順を記載した専用ページにリンクしています。
...
# 2. 使い始める
---
Telechips Model Zoo が提供するニューラルネットワークモデルを AI-G ボードで実行するには、次の手順に従ってください。
<br/><br/><br/>
...
## 2.2 目的のモデルを AI-G にコピー
---
モデルフォルダ (yolov5m) 全体を AI-G にコピーします。
フォルダをそのままコピーすることも、次の例のように、以前にリリースされたニューラルネットワークフォルダで使用されている形式に合わせて yolov5m_quantized にリネームすることもできます。
各フォルダには、必要な出力ファイル (.so、.bin など) が含まれています。
...
## 2.3 tcnputestapp または tcnnapp を使用したモデルの実行:
---

# 5. 照度センサー (BH 1750, SZH-SSBH-011)
### 5.1 BH 1750
* 電源電圧 **3~5V**
* データ範囲 : 0~65535

以下のコードは、i2c 通信を通じて照度センサーのデータ値を取得し、0 から 100 までの比率で返すサンプルコードです。
```
import smbus
import time
import os
...
以下のコードは、センサーのデータピンにおいて、照度が低くデータ値が 0 のときに消灯し、照度が高くデータ値が 1 のときに点灯するサンプルコードです。
このセンサーは、暗いときにデータ値が 1、明るいときに 0 になります。
```
import os
import time

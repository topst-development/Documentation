# 1. 소개
---
본 문서는 U-Boot를 사용한 임베디드 시스템 복구, initramfs를 이용한 부팅, 그리고 root 비밀번호 재설정에 대한 가이드라인을 제공합니다.

본 가이드는 다음 항목을 다룹니다:

- U-Boot 복구 원리

- USB 및 initramfs를 이용한 부팅
...
# 2. U-Boot 복구 원리
---
eMMC 파일 시스템에서 부팅하는 대신, U-Boot를 사용하여 initramfs 이미지를 통해 시스템을 시작합니다. 이 방식은 루트 파일 시스템이 손상되었거나 접근할 수 없는 경우에 특히 유용합니다. 시스템이 initramfs로 부팅된 후 eMMC 루트 파일 시스템을 마운트하면, 관리자는 파일 수정이나 자격 증명 재설정과 같은 복구 작업을 수행할 수 있습니다.
</br><br/><br/><br/>

# 3. U-Boot와 initramfs를 이용한 부팅
---
U-Boot는 부팅 가능한 이미지(initramfs, dtb 등)를 메모리에 로드하고 실행하여 시스템을 초기화하는 부트로더입니다. 

복구 상황에서는 루트 파일 시스템 대신 initramfs로 시스템을 부팅할 수 있게 하여, 시스템에 접근하기 위한 독립적인 환경을 제공합니다.
...
- D3-G YP 빌드 결과물이 위치한 다음 디렉터리로 이동합니다:
...
## 3.2 U-Boot 모드에서의 부팅 절차
---
USB를 D3-G에 삽입하고 전원을 켭니다. U-Boot 모드로 진입한 후 다음 명령어를 실행하십시오:

**u-boot 모드로 진입하려면 장치의 전원을 켠 후 3초 이내에 enter 키를 누르십시오.**
...
```
u-boot 환경에서 booti 0x20000000 - 0x30000000 명령을 통해 Linux 커널 이미지를 부팅합니다.
```
...
# 4. root 비밀번호 재설정
---
initramfs 셸에 진입한 후, 아래 단계를 따라 eMMC를 마운트하고 저장된 root 비밀번호를 재설정하십시오.
...
**lsblk 또는 blkid를 사용하여 올바른 파티션인지 반드시 확인하십시오.**
```
$ mkdir -p /mnt/part3
...
## 4.2 shadow 파일 수정
---
root 비밀번호는 /etc/shadow 파일에 해시 형태로 저장됩니다. 비밀번호를 삭제하려면 다음을 실행하십시오:
...
## 4.3 마무리 및 재부팅
---
변경 사항이 디스크에 기록되도록 하려면 다음을 실행하십시오:

```
$ sync
$ reboot
```
이제 비밀번호 없이 root로 로그인할 수 있으며, passwd를 사용하여 새 비밀번호를 설정할 수 있습니다.

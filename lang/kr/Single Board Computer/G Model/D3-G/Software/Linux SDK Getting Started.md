# 1. 소개
---
이 문서는 호스트 환경 설정, SDK 빌드, 펌웨어 다운로더 사용, Ubuntu 다운로드 등 D3-G SDK를 빌드하기 위한 지침을 제공합니다.  
...
# 2. 호스트 환경 설정
---
이 장에서는 호스트 PC 환경을 설정하는 방법을 Windows와 Ubuntu로 나누어 설명합니다.
</br><br/><br/>

## 2.1 Windows 환경 
---
이 문서에서는 Windows PC에서 Linux를 사용하기 위해 Windows Subsystem for Linux(WSL)를 설정하는 방법을 설명합니다.
D3-G Linux SDK는 Yocto Project를 기반으로 하므로 D3-G SDK의 Linux 버전은 Yocto Project를 따릅니다.
다른 버전의 Linux를 설치할 수도 있지만, 이 문서에서는 Ubuntu 22.04를 기반으로 하는 D3-G Linux SDK를 설명합니다.
...
6. Ubuntu 설치 중에 원하는 사용자 이름을 선택합니다.
</br><br/>

### 2.1.2 WSL2를 통한 Ubuntu 접속
Windows 명령 프롬프트를 열고 다음 명령을 입력하여 Ubuntu에 접속합니다.
Ubuntu에 접속하면 기본적으로 /mnt/c/Users/[username] 디렉터리에서 시작합니다.
```
wsl  // access ubuntu 
ls   // check contents in your directory
```
결과를 확인하려면 그림 2.1을 참조하십시오(결과는 시스템에 따라 다를 수 있습니다).
...
Ubuntu에 접속한 후에는 SSH와 Samba 같은 추가 유틸리티를 사용하여 개발 환경을 더 편리하게 구성할 수 있습니다. SSH와 Samba를 사용하면 원격 컴퓨터에서 명령을 실행하고 다른 컴퓨터로 파일을 복사할 수 있습니다.
 - 다음 단계를 수행하려면 호스트 PC가 네트워크에 연결되어 있어야 합니다. 다음 명령을 사용하여 네트워크 상태를 확인하십시오.
  ```
  $ sudo apt-get update
...
```
SSH와 Samba를 설치한 후 환경에 맞게 각 프로그램을 설정하십시오.
</br><br/>

### 2.1.5 유틸리티 설치

다음 명령을 사용하여 필요한 모든 유틸리티를 한 번에 설치합니다. Yocto Project를 사용하려면 호스트 PC(개인 컴퓨터 또는 개발 서버)에 다음 유틸리티가 설치되어 있어야 합니다.
...
Repo가 이미 설치되어 있으면 다시 설치하지 않고 사용할 수 있습니다.  
Repo를 설치하기 전에 Python 3.6 이상 버전이 설치되어 있는지 확인하십시오.
...
'/usr/bin/env 'python' no such file or directory' 오류 메시지가 표시되면 다음 명령을 사용하여 'python'을 'python3'에 링크하십시오.

```
$ sudo ln -sf /usr/bin/python3 /usr/bin/python
```
Repo 오류가 발생하면 다음 명령을 사용하여 최신 버전을 다운로드한 후 /usr/bin/ 폴더에 배치하십시오.
...
## 2.2 Linux 환경
---
이 장에서는 호스트 OS로 Ubuntu를 사용하는 경우의 설정 과정을 설명합니다.
</br><br/>

### 2.2.1 환경 설정
다음 장(2.2.2 ~ 2.2.5)은 Ubuntu 터미널에서 실행해야 합니다. 터미널을 열려면 단축키 [Ctrl + Alt + T]를 사용하십시오.
<br/><br/>
...
다음 명령을 사용하여 필요한 모든 유틸리티를 한 번에 설치합니다. Yocto Project를 사용하려면 호스트 PC(개인 컴퓨터 또는 개발 서버)에 다음 유틸리티가 **반드시** 설치되어 있어야 합니다.
****
...
### 2.2.6 Telechips USB 장치를 위한 Udev Rules
...
# 3. 이미지 빌드 가이드
---
이 장에서는 호스트 PC에 설치된 Ubuntu OS(WSL인지 로컬 Ubuntu 설치인지에 관계없이)를 기준으로 안내합니다. D3-G에 업로드할 이미지는 Yocto Project를 사용하여 빌드되므로 빌드 과정은 Ubuntu 환경에서 수행해야 합니다.
</br></br>

## 3.1 SDK 빌드 준비
---
D3-G Linux SDK는 Yocto Project 4.0 Kirkstone 을 기반으로 합니다. 따라서 D3-G Linux SDK를 사용하려면 호스트 PC에 Yocto Project 환경을 구성해야 합니다. SDK, source-mirror 및 툴을 다운로드하려면 필요한 유틸리티를 설치해야 합니다. 이미지를 원활하게 빌드하려면 PC에 **최소 60 GB의 사용 가능한 저장 공간**과 **최소 16 GB의 RAM**이 있어야 합니다.
...
Linux 이미지를 만들기 위해 Open Embedded 프로젝트인 Poky와 ***bitbake***를 빌드 시스템으로 조합하여 사용합니다.  
...
---
그림 3.1은 Yocto Project의 작업 과정을 보여줍니다. 메타데이터를 기반으로 업스트림에서 소스를 다운로드하여 빌드할 수 있습니다. 빌드가 완료되면 패키지, 이미지, SDK가 결과물로 제공됩니다.
...
## 3.4 D3-G SDK의 구성
---
다음은 당사가 구성한 Yocto Project의 구성 요소입니다.
...
공식 TOPST git에서 D3-G SDK를 다운로드하려면 이메일과 이름을 설정하십시오.
1. 다음 명령을 입력합니다.
```
vi ~/.gitconfig
```
...
<p align="center"><strong>그림 3.2 최종 사용자 사용권 계약</strong></p>
...
<p align="center"><strong>그림 3.3 'Proceed to confirm'으로 이동</strong></p>
...
<p align="center"><strong>그림 3.4 동의 화면 </strong></p>
...
topst-build.sh는 D3-G 및 AI-G용 이미지를 빌드하는 데 필요한 핵심 환경을 설정하는 셸 스크립트입니다. 다음 명령을 실행한 후, D3-G에 메인 OS를 설치하기 위한 빌드 환경을 준비하려면 옵션 2를 선택하십시오.
...
## 3.7 Firmware Downloader (FWDN) 이미지 생성 
---
이 옵션은 D3-G 플랫폼 이미지를 위해 바이너리들을 하나의 이미지로 결합합니다.

**'output_d3g.fai' 빌드 이미지**와 **FWDN 툴**이 포함된 **output_d3g.fwdn.zip** 파일이 다음 경로에 생성됩니다:
...
# 4. 펌웨어 다운로드
---
이 장에서는 ***FWDN***을 사용하여 D3-G에 펌웨어를 다운로드하고 Linux 콘솔에 로그인하는 방법을 설명합니다.  
***FWDN V8***은 Windows 10(11) 64비트 및 Linux 환경 모두에서 펌웨어를 다운로드하기 위한 PC 툴입니다. 이 장에서는 Windows 및 Linux 환경에서 다운로드하는 경우를 설명합니다.
...
1. 부팅 모드 스위치를 USB 부팅 모드로 설정하십시오.
2. Windows 프롬프트 또는 Linux 콘솔을 여십시오.
3. ***FWDN V8***을 보드에 연결하십시오.
4. fai 파일을 다운로드하십시오.

<br/><br/><br/>

## 4.2 USB 부팅 모드로 D3-G 보드와 호스트 PC 연결
---
Firmware Downloader (FWDN)는 호스트 PC와의 USB 통신을 통해 D3-G에 ROM 이미지를 기록합니다. 

D3-G에는 하나의 Boot Mode 버튼이 있으며 두 가지 유형의 부팅 모드를 지원합니다. 이 가이드는 FWDN 모드에 중점을 둡니다.

- USB Boot Mode (FWDN Mode) : 호스트 PC에서 FWDN 툴을 사용하여 ROM 이미지를 기록할 때 사용합니다 

- eMMC Boot Mode : eMMC 장치에 저장된 ROM 이미지를 사용하여 D3-G를 부팅할 때 사용합니다 

**참고**: USB Type-C FWDN 포트는 firmware downloader (FWDN)에 사용됩니다. 
...
2. USB Type-C 케이블 1개를 준비하십시오. 

3. USB 부팅 모드로 진입하려면 FWDN 스위치를 누른 상태에서 D3-G 보드에 전원 케이블을 연결하십시오.
   - 자세한 내용은 사이드바의 Hardware 섹션에 있는 **"2. Boot Mode"**를 참조하십시오.
...
<p align="center"><strong>그림 4.1 USB C-Type 케이블을 사용한 D3-G 보드와 호스트 PC의 연결 </strong></p>

<br/><br/>

### 4.2.1 VTC 드라이버 설치 방법 (Windows/Ubuntu)
호스트 PC에서 관리자 권한으로 실행하여 Vendor Telechips Certification (VTC) 드라이버([telechips driver](https://drive.google.com/file/d/1muQnY8kuKxDsy3p3FUiQqcG34Zjk-mnR/view?usp=sharing)에서 확인 가능)를 설치하십시오. 위와 같이 FWDN 모드에서 USB를 연결하면 그림 4.2 및 그림 4.3과 같이 Telechips VTC USB 드라이버가 설정됩니다.
...
**참고**: VTC 드라이버 V5.0.0.14 이상을 사용하십시오. 버전을 확인하려면 Windows 환경에서 장치 관리자를 확인하십시오.  

<br/><br/><br/>

## 4.3 FWDN 다운로드 준비
---
FWDN을 실행하기 전에 Ubuntu (WSL2) 환경에서 생성한 이미지와 툴을 Windows 환경으로 전송하십시오.


1. "output_d3g.fwdn.zip"의 압축을 푸십시오.   
    ```
    $ cd ~/topst-sdk
    $ mkdir images
    ...
    ```
2. "images" 폴더를 Windows C 드라이브로 복사하십시오.  
    ```
    $ cd ..
    $ cp -r ./images /mnt/c/
    ```
...
---
1. Powershell을 실행하고 "C:\images\"로 이동하십시오.
```
$ cd C:\images 
```

2. 펌웨어 다운로드를 시작하려면 **.\fwdn.bat** 명령을 입력하십시오. “fwdn.bat”은 FWDN V8을 사용하여 펌웨어를 자동으로 다운로드하는 실행 파일입니다. 
...
이제 D3-G를 부팅할 준비가 되었습니다. 장치와의 통신을 시작하려면 5장을 참조하십시오.


<br/><br/><br/><br/>

# 5. D3-G 보드와 호스트 PC 연결
---
이 장에서는 펌웨어 다운로드 및 시리얼 통신을 위해 UART를 통해 호스트 PC를 D3-G 보드에 연결하는 방법을 설명합니다.

<br/><br/><br/>

## 5.1 UART를 이용한 D3-G 보드 연결 
---
...
3. 호스트 PC와 D3-G 보드의 UART 핀을 연결하십시오. USB-to-TTL 케이블을 사용하십시오. 
4. 검은색 케이블을 GND 핀에 연결하십시오. 
5. 흰색 케이블(RXD)을 UART 핀의 TX 핀에 연결하고, 녹색 케이블(TXD)을 UART 핀의 RX 핀에 연결합니다.
6. 터미널 에뮬레이터 애플리케이션을 실행합니다.
7. PC에서 장치 관리자를 열고 UART 장치에 할당된 포트 번호를 확인하십시오.
8. 터미널 에뮬레이터에서 확인한 포트 번호를 Serial line 필드에 입력하십시오. **Speed** (bps)는 115200으로, **Flow control**은 **None**으로 설정하십시오.
9. 전원 케이블을 연결하십시오. 그러면 D3-G 보드가 기본 eMMC 부팅 모드로 부팅됩니다.
...
<p align="center"><strong>그림 5.2 연결된 화면 (ID와 비밀번호는 topst입니다)</strong></p><br/>

<br/><br/><br/>

# 6. Ubuntu OS 파티션 크기 조정
---
당사는 Ubuntu OS도 제공합니다.
이 장에 따라 진행하면 Ubuntu 이미지를 다운로드하여 보드에 업로드하고 할당된 eMMC 저장 용량을 확장할 수 있습니다.
...
**다운로드 :**  
-	[여기에서 다운로드](https://drive.google.com/file/d/1oc2qwaXUt6-QDME3s5WXKVHzAg4xqVyc/view?usp=drive_link)
<br>
-	자세한 내용은 당사의 [github 페이지](https://github.com/topst-development)를 방문하십시오.
...
다른 OS의 릴리스에 대한 정보는 TOPST 커뮤니티를 참조하십시오.  
...
FWDN이 완료된 후 FWDN 포트에서 USB Type-C 케이블을 제거하고 전원 케이블을 제거하십시오. 

<br/><br/><br/>

## 6.3 eMMC 저장 공간 크기 조정 (D3-G 전용)
---
보드에 로그인하여 부팅한 후에는 먼저 eMMC 저장 공간의 크기를 조정하는 것을 권장합니다.
eMMC 저장 공간 크기 조정을 위해 아래 단계를 따르십시오.

1. 파티션 크기와 레이아웃을 수정하려면 다음 명령을 사용하십시오.
     ```
     $ parted
     ```

2. GUID Partition Table(GPT)을 확장하십시오. 
    ```
    $ rescue
    $ Fix 
    $ 0 
    $ 100%
    ```
3. p (print) 명령을 사용하여 파티션 유형이 ext4인지 확인하십시오. 
   ```
   $ p
   ```
4. 파티션 4의 크기를 조정하십시오.
    ...
    ```
5. 보드를 재부팅하십시오.
6. 파티션 4의 ext4 파일 시스템 크기를 조정하십시오.
    ```
    $ resize2fs /dev/mmcblk0p4
    ```
7. 다음 명령을 사용하여 변경된 파티션 크기를 확인하십시오.
   ```
   $ df -h
   ```

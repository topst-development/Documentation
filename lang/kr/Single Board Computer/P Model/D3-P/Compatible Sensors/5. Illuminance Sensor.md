# 5. 조도 센서 (BH 1750, SZH-SSBH-011)
### 5.1 BH 1750
* 전원 공급 **3~5V**
* 데이터 범위 : 0~65535

다음 코드는 i2c 통신을 통해 조도 센서의 데이터 값을 가져와 0에서 100까지의 비율로 반환하는 예제 코드입니다.
```
import smbus
import time
import os
...
다음 코드는 센서의 데이터 핀을 사용하여 조도가 낮아 데이터 값이 0일 때 꺼지고, 조도가 높아 데이터 값이 1일 때 켜지는 예제 코드입니다.
이 센서는 어두울 때 데이터 값이 1이고 밝을 때 0입니다.
```
import os
import time

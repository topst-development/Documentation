...
## 1.1 USB 부트 모드로 D3-G 보드와 호스트 PC 연결
---
Firmware Downloader(FWDN)는 호스트 PC와의 USB 통신을 통해 D3-G에 ROM 이미지를 기록합니다. 

D3-G에는 Boot Mode 버튼이 하나 있으며 두 가지 부팅 모드를 지원합니다. 본 가이드는 FWDN 모드를 중심으로 설명합니다.

- USB Boot Mode (FWDN Mode) : 호스트 PC의 FWDN 도구를 사용하여 ROM 이미지를 기록할 때 사용합니다 
...
1. 호스트 PC에 VTC 드라이버가 설치되어 있는지 확인하십시오. VTC 드라이버가 설치되어 있지 않으면 1.2장에 설명된 대로 설치하십시오.  

2. USB Type-C 케이블 1개를 준비하십시오. 

3. USB 부트 모드로 진입하려면 FWDN 스위치를 누른 상태에서 D3-G 보드에 전원 케이블을 연결하십시오.
...
<p align="center"><strong>그림 1.1 USB C-Type 케이블을 사용한 D3-G 보드와 호스트 PC 연결 </strong></p>

<br/><br/>

## 1.2 VTC 드라이버 설치 방법 (Windows/Ubuntu)
관리자 권한으로 실행하여 호스트 PC에 Vendor Telechips Certification (VTC) 드라이버([telechips 드라이버](https://drive.google.com/file/d/1muQnY8kuKxDsy3p3FUiQqcG34Zjk-mnR/view?usp=sharing)에서 제공)를 설치하십시오. 위와 같이 FWDN 모드에서 USB를 연결하면 그림 1.2 및 그림 1.3과 같이 Telechips VTC USB 드라이버가 설정됩니다.
...
3. fwdn.bat을 클릭합니다. “fwdn.bat”은 ***FWDN V8***을 사용하여 펌웨어를 자동으로 다운로드하는 실행 파일입니다. 
...
2. D3-G Ubuntu Desktop 이미지를 다운로드합니다
...
2. D3-G Ubuntu Headless 이미지를 다운로드합니다
...
### 1.4.1 D3-G 이미지 압축 해제
---
1.3절에서 다운로드한 D3-G 이미지를 Linux 시스템에서 압축 해제하십시오.

<br/><br/><br/>

### 1.4.2 Telechips USB 장치용 Udev 규칙
---
...
---
본 장에서는 펌웨어 다운로드 및 시리얼 통신을 위해 UART를 통해 호스트 PC를 D3-G 보드에 연결하는 방법을 설명합니다.
...
8. 장치 관리자에서 확인한 포트 번호를 터미널 에뮬레이터의 Serial line 필드에 입력하십시오. **Speed** (bps)를 115200으로 설정하고 **Flow control을 None으로 설정하십시오.**
9. 전원 케이블을 연결하십시오. 그러면 D3-G가 기본 eMMC 부트 모드로 부팅됩니다.

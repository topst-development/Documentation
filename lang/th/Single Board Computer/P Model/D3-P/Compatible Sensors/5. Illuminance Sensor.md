# 5. เซ็นเซอร์วัดความสว่าง (BH 1750, SZH-SSBH-011)
### 5.1 BH 1750
* แหล่งจ่ายไฟสำหรับ **3~5V**
* ช่วงข้อมูล : 0~65535

โค้ดต่อไปนี้เป็นโค้ดตัวอย่างที่อ่านค่าข้อมูลของเซ็นเซอร์วัดความสว่างผ่านการสื่อสาร i2c และส่งคืนค่าเป็นอัตราส่วนตั้งแต่ 0 ถึง 100
```
import smbus
import time
import os
...
```
### 5.2 SZH-SSBH-011
* แหล่งจ่ายไฟสำหรับ **3.3V, 5V**

โค้ดต่อไปนี้เป็นโค้ดตัวอย่างที่จะดับเมื่อค่าข้อมูลเป็น 0 เนื่องจากความสว่างต่ำที่ขาข้อมูลของเซ็นเซอร์ และจะติดเมื่อค่าข้อมูลเป็น 1 เนื่องจากความสว่างสูง
เซ็นเซอร์ตัวนี้จะมีค่าข้อมูลเป็น 1 เมื่อมืด และเป็น 0 เมื่อสว่าง
```
import os
import time

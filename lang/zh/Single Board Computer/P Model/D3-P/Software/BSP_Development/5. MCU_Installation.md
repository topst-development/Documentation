...
图 5.1 显示了 TCC8050 MCU BSP 中的软件组件。TCC8050 MCU BSP 由设备驱动程序、实时操作系统 (RTOS)、系统适配层 (SAL) 和示例应用程序组成。MCU BSP 按用途分为三种类型（Sample、3rd party 和 TC Support）。Sample 是用于测试设备驱动程序的示例代码。您可以在实现自己的解决方案时参考该示例代码，但其质量不作保证。MCU BSP 包含 RTOS 等第三方软件，但本 MCU BSP 不为 RTOS 本身提供技术支持。设备驱动程序和 SAL 接口由 Telechips 提供支持 (TC Support)。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/e4b3a9e6-6c34-4156-a7ef-4d25bbde8a8a">
</p>
<p align="center"><strong>图 5.1 TCC8050 MCU BSP 中的软件组件</strong></p>
...
<p align="center"><strong>图 5.2 目录结构</strong></p>

① documents：TCC8050 MCU BSP 的所有文档

② scripts：用于 Trace32 等 JTAG 调试器的调试脚本

③ sources：源代码

1. app.drivers：应用程序驱动程序。这些驱动程序不直接访问硬件。
2. app.sample：示例应用程序
3. build：构建环境和实用工具
4. core：汇编核心文件
5. dev.drivers：设备驱动程序
6. os：操作系统
7. sal：系统适配层

④ tools：用于 FWDN 和 FW 升级的工具
...
构建 MCU 源代码后，使用 Firmware Downloader (***FWDN***) 创建可加载到 TOPST D3（开放平台开发板）的映像。
...
TCC8050 MICOM 子系统具有 5 通道 UART，每个 UART 通道由一个 ARM® PrimeCell™ UART (PL011) 控制器和一个 ARM® PrimeCell™ DMA 控制器 (PL081) 组成。通过 UART 通信收发的数据可由 UART 专用 DMA 控制器与内部存储器（SRAM-0 或 SRAM-1）之间进行传输。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/f52d2fc1-91c4-4fd1-a37f-3133ecc59be4">
</p>
<p align="center"><strong>图 5.3 UART 框图</strong></p>
...
- 使用可编程的 UART 输入/输出
- 独立的 32×8 发送和 32×12 接收先进先出 (FIFO) 存储器缓冲区，用于减少 CPU 中断。
- 可编程的 FIFO 禁用功能，使深度为 1 字节。
- 可编程的波特率发生器。该功能可对高于 3.6864 MHz 的参考时钟频率进行分频。
- 标准异步通信位（起始位、停止位和奇偶校验位）。
- 可对发送 FIFO、接收 FIFO、接收超时、调制解调器状态和错误状态中断进行独立屏蔽。
- 支持直接存储器访问 (DMA)。
- 错误起始位检测
- 线路中断的产生与检测。
- 可编程的硬件流控制：CTS 和 RTS 的调制解调器控制功能
- 完全可编程的串行接口特性：
- 数据可编程为 5、6、7 或 8 位
- 偶校验、奇校验、固定校验或无校验位的产生与检测
- 产生 1 位或 2 位停止位
- 波特率产生，最高可达 UARTCLK/16
- 用于唯一标识 UART 的标识寄存器。操作系统可利用这些寄存器自动完成配置。
...
- 存储器到存储器、存储器到 UART、UART 到存储器的传输
- DMA 控制器具有两个通道。
- 通过使用链表支持分散或聚集 DMA。这意味着源区域和目标区域无需占用连续的存储器区域。
- 一个用于传输数据的 AHB 总线主设备。该接口在产生 DMA 请求时传输数据
- 32 位 AHB 主总线宽度。
- 源地址和目标地址支持递增或非递增寻址。
- 可编程的 DMA 突发大小。可通过编程设置 DMA 突发大小以更高效地传输数据。通常，突发大小设置为外设中 FIFO 大小的一半。
- 每个通道具有内部四字 FIFO。
- 支持大端和小端模式。DMA 控制器在复位时默认为小端模式。
- 合并的 DMA 错误中断请求和 DMA 计数中断请求。当发生 DMA 错误或 DMA 计数达到 0 时，可向处理器产生中断。这通常用于指示传输已完成。以下三个中断请求信号实现该功能：
- INTTC 在传输完成时发出信号。
   * **注意**：INTTC 信号不会输入到中断控制器，但可通过状态寄存器查看其状态。
- INTERR 在发生错误时发出信号。
   * **注意**：INTERR 信号不会输入到中断控制器，但可通过状态寄存器查看其状态。
- INTR 合并了 INTTC 和 INTERR 两个中断请求信号。INTR 信号会输入到中断控制器。
- 中断屏蔽：可屏蔽 DMA 错误和 DMA 终止计数中断请求。
- 原始中断状态：可在屏蔽之前读取 DMA 错误和 DMA 计数的原始中断状态。

<br/>

### 5.4.1.2 调试 UART 初始化

以下源代码是有关如何为 MCU 调试设置 UART 的基本信息。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/c3e8b3e2-437e-43e4-8480-5e3fa4e075c8">
</p>
<p align="center"><strong>图 5.4 MICOM / UART|JTAG HEADER 原理图</strong></p>
...
控制器局域网 (CAN) 按照 ISO 11898-1（与 Bosch CAN 协议规范 2.0 part A、B 相同）执行通信。此外，CAN 还支持按照 CAN 灵活数据速率 (FD) 协议规范 1.0 进行通信。

所有处理消息的功能均由 Rx Handler 和 Tx Handler 实现。

Rx Handler 负责管理消息验收过滤、将接收到的消息从 CAN Core 传输到 Message RAM，并提供接收消息的状态信息。

Tx Handler 负责将发送消息从 Message RAM 传输到 CAN Core，并提供发送状态信息。它实现了与时间调度和全局系统时间相关的所有功能。

验收过滤通过最多 128 个过滤元素的组合来实现，其中每个元素均可配置为范围、位掩码或专用 ID 过滤器。

TCC8050 MICOM 子系统中共集成了三个 CAN FD 控制器。CPU 可通过 AHB to APB host I/F 配置各个 CAN 控制器。每个 CAN FD 控制器通过 AHB 总线矩阵访问集成在 MICOM 子系统中的 SRAM-0 或 SRAM-1，并将 SRAM-0 或 SRAM-1 用作消息存储器。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/e4ab5446-9833-482e-b73f-b968168e9a8f">
</p>
<p align="center"><strong>图 5.5 CAN FD 控制器框图</strong></p>
...
- 符合 CAN 协议版本 2.0 part A、B (ISO 11898-1)、Bosch CAN FD 规格 V1.0、ISO 16845
- 支持最多 64 个数据字节的 CAN FD
- CAN 错误检测与记录 (Stuff/Bit/Format/CRC/ACK)
- 改进的验收滤波
- 两个可配置的接收 FIFO
- 接收高优先级消息时提供独立的信号通知
- 最多 64 个专用接收缓冲区
...
<p align="center"><strong>图 5.6 CAN GPIO</strong></p>
...
如上面的 CAN GPIO 图所示，在下面的代码中将 GPIO PIN 应用于 CAN0、CAN1 和 CAN2 端口。

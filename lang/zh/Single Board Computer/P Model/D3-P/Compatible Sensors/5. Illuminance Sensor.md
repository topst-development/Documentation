# 5. 照度传感器 (BH 1750, SZH-SSBH-011)
### 5.1 BH 1750
* 供电电压 **3~5V**
* 数据范围 : 0~65535

以下代码是通过 i2c 通信获取照度传感器的数据值,并以 0 到 100 的比例返回的示例代码。
```
import smbus
import time
import os
...
以下代码是一段示例代码,通过传感器的数据引脚,在照度较低、数据值为 0 时熄灭,在照度较高、数据值为 1 时点亮。
该传感器在黑暗时数据值为 1,在明亮时为 0。
```
import os
import time

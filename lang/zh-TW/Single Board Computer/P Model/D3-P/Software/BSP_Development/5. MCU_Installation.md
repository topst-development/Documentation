...
圖 5.1 顯示 TCC8050 MCU BSP 中的軟體元件。TCC8050 MCU BSP 由裝置驅動程式、即時作業系統（Real Time Operating System, RTOS）、系統調適層（System Adaptation Layer, SAL）與範例應用程式所組成。MCU BSP 依用途分為三種類型（Sample、3rd party 與 TC Support）。Sample 為用於測試裝置驅動程式的範例程式碼。您可在實作自有解決方案時參考此範例程式碼，但其品質不受保證。MCU BSP 包含 RTOS 等第三方軟體，但本 MCU BSP 不對 RTOS 本身提供技術支援。裝置驅動程式與 SAL 介面由 Telechips 提供支援（TC Support）。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/e4b3a9e6-6c34-4156-a7ef-4d25bbde8a8a">
</p>
<p align="center"><strong>圖 5.1 TCC8050 MCU BSP 中的軟體元件</strong></p>
...
<p align="center"><strong>圖 5.2 目錄結構</strong></p>

① documents：TCC8050 MCU BSP 的所有文件

② scripts：適用於 Trace32 等 JTAG 除錯器的除錯指令碼

③ sources：原始程式碼

1. app.drivers：應用程式驅動程式。這些驅動程式不會直接存取硬體。
2. app.sample：範例應用程式
3. build：建置環境與公用程式
4. core：組合語言核心檔案
5. dev.drivers：裝置驅動程式
6. os：作業系統
7. sal：系統調適層
...
建置 MCU 原始碼後，請使用韌體下載工具（***FWDN***）建立可載入至 TOPST D3（開放平台開發板）的映像檔。
...
TCC8050 MICOM 子系統具備 5 通道 UART，每個 UART 通道由一個 ARM® PrimeCell™ UART（PL011）控制器與一個 ARM® PrimeCell™ DMA 控制器（PL081）組成。透過 UART 通訊傳送／接收的資料，可由 UART 專用 DMA 控制器與內部記憶體（SRAM-0 或 SRAM-1）之間互相傳輸。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/f52d2fc1-91c4-4fd1-a37f-3133ecc59be4">
</p>
<p align="center"><strong>圖 5.3 UART 方塊圖</strong></p>
...
- 使用可程式化的 UART 輸入／輸出
- 獨立的 32×8 傳送與 32×12 接收先進先出（First-In, First-Out, FIFO）記憶體緩衝區，可減少 CPU 中斷。
- 可程式化停用 FIFO，使其深度為 1 位元組。
- 可程式化的鮑率產生器。可對大於 3.6864 MHz 的參考時脈頻率進行分頻。
- 標準的非同步通訊位元（起始、停止與同位）。
- 可獨立遮罩傳送 FIFO、接收 FIFO、接收逾時、數據機狀態與錯誤狀況等中斷。
- 支援直接記憶體存取（Direct Memory Access, DMA）。
- 錯誤起始位元偵測
- 線路中斷（break）訊號的產生與偵測。
- 可程式化的硬體流量控制：CTS 與 RTS 的數據機控制功能
- 完全可程式化的序列介面特性：
- 資料可程式化為 5、6、7 或 8 位元
- 偶同位、奇同位、固定同位（stick）或無同位位元的產生與偵測
- 產生 1 或 2 個停止位元
- 鮑率產生，最高可達 UARTCLK/16
- 可唯一識別 UART 的識別暫存器。作業系統可利用這些暫存器自動完成自身設定。

<br/>

### 5.4.1.1.2 UART DMA 控制器

UART DMA 控制器的功能特性如下：

- 記憶體對記憶體、記憶體對 UART、UART 對記憶體的傳輸
- DMA 控制器具有兩個通道。
- 透過連結串列支援 scatter 或 gather DMA。這表示來源與目的區域不必佔用連續的記憶體區域。
- 一個用於傳輸資料的 AHB 匯流排主控端。當 DMA 要求產生時，此介面即傳輸資料
- 32 位元 AHB 主控匯流排寬度。
- 來源與目的端可採用遞增或非遞增定址。
- 可程式化的 DMA 叢發（burst）大小。可設定 DMA 叢發大小以更有效率地傳輸資料。通常叢發大小會設為周邊裝置中 FIFO 大小的一半。
- 每個通道內建四個字組（word）的 FIFO。
- 支援大端序與小端序。DMA 控制器在重置時預設為小端序模式。
- 合併的 DMA 錯誤與 DMA 計數中斷要求。當發生 DMA 錯誤或 DMA 計數達到 0 時，可對處理器產生中斷。此機制通常用於指示傳輸已完成。共有三個中斷要求訊號負責此功能：
- INTTC 於傳輸完成時發出訊號。
   * **註**：INTTC 訊號不會輸入至中斷控制器，但可透過狀態暫存器檢查其狀態。
- INTERR 於發生錯誤時發出訊號。
   * **註**：INTERR 訊號不會輸入至中斷控制器，但可透過狀態暫存器檢查其狀態。
- INTR 結合了 INTTC 與 INTERR 兩個中斷要求訊號。INTR 訊號會輸入至中斷控制器。
- 中斷遮罩：您可以遮罩 DMA 錯誤與 DMA 終止計數中斷要求。
- 原始中斷狀態：您可以在遮罩之前讀取 DMA 錯誤與 DMA 計數的原始中斷狀態。

<br/>

### 5.4.1.2 除錯 UART 初始化

下列原始程式碼為設定 UART 以進行 MCU 除錯的基本資訊。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/c3e8b3e2-437e-43e4-8480-5e3fa4e075c8">
</p>
<p align="center"><strong>圖 5.4 MICOM / UART|JTAG HEADER 電路圖</strong></p>
...
控制器區域網路（Controller Area Network, CAN）依據 ISO 11898-1（與 Bosch CAN 通訊協定規格 2.0 part A、B 相同）執行通訊。此外，CAN 亦支援依據 CAN 彈性資料速率（Flexible Data-rate, FD）通訊協定規格 1.0 的通訊。

所有處理訊息的功能均由 Rx Handler 與 Tx Handler 實作。

Rx Handler 負責管理訊息接受過濾、將接收到的訊息由 CAN Core 傳送至 Message RAM，並提供接收訊息的狀態資訊。

Tx Handler 負責將傳送訊息由 Message RAM 傳送至 CAN Core，並提供傳送狀態資訊。它會實作所有與時間排程及全域系統時間相關的功能。

接受過濾功能是由最多 128 個過濾元素組合而成，每個元素皆可設定為範圍、位元遮罩或專用 ID 過濾器。

TCC8050 MICOM 子系統共整合三個 CAN FD 控制器。CPU 可透過 AHB 至 APB 主控端 I/F 設定各個 CAN 控制器。每個 CAN FD 控制器透過 AHB 匯流排矩陣存取整合於 MICOM 子系統中的 SRAM-0 或 SRAM-1，並將 SRAM-0 或 SRAM-1 作為訊息記憶體使用。

<p align="center">
    <img src="https://github.com/topst-development/Documentation/assets/161264431/e4ab5446-9833-482e-b73f-b968168e9a8f">
</p>
<p align="center"><strong>圖 5.5 CAN FD 控制器方塊圖</strong></p>
...
- 符合 CAN 通訊協定版本 2.0 part A、B（ISO 11898-1）、Bosch CAN FD 規格 V1.0、ISO 16845
- 支援最多 64 個資料位元組的 CAN FD
- CAN 錯誤偵測與記錄（Stuff/Bit/Format/CRC/ACK）
- 改良的接受過濾功能
- 兩個可設定的接收 FIFO
- 接收高優先權訊息時提供獨立的訊號通知
- 最多 64 個專用接收緩衝區
...
<p align="center"><strong>圖 5.6 CAN GPIO</strong></p>
...
如上方 CAN GPIO 圖所示，請在下列程式碼中將 GPIO 腳位套用至 CAN0、CAN1 與 CAN2 連接埠。

# 1. Introduction
This document provides usage guidance for the sample and test applications included in the TOPST AI-G SDK. These applications are designed to help users understand and evaluate AI inference capabilities using the onboard NPU of the TOPST AI-G board.  
...
This section describes the three sample/test applications provided in the TOPST AI-G SDK. Each application is designed to help users evaluate and utilize the AI capabilities of the TOPST AI-G board. The applications cover various scenarios such as NPU inference testing and real-time camera input.
<br/><br/><br/>

## 2.1 tcnnapp
tcnnapp is a sample AI application that demonstrates how to perform image-based inference using the NPU on the TOPST AI-G platform. It supports various input/output modes and showcases object detection using pre-trained models such as YOLOv5s and MobileNetv2.
<br/><br/>
...
The RTPM toolkit  is a GUI-based tool developed in Python that monitors output from the TOPST AI-G board in a PC environment. It allows you to observe inference results as well as system performance merics such as CPU, NPU, and memory usage.  
...
## 2.2 tcnncameraapp
tcnncameraapp is a sample application that allows you to test and preview camera modules supported on the TOPST AI-G platform. It is built on the Video4Linux2 (V4L2) framework and enables real-time camera preview through a connected MIPI CSI interface and DSI display.  
...
## 2.3 tcnputestapp
tcnputestapp is a sample application that verifies the basic functionality of the Neural Processing Unit (NPU) on the TOPST AI-G platform. It utilizes the tcndnpulib library to directly test NPU execution with pre-compiled neural network models. This application is primarily intended for internal validation, debugging, and performance testing of the NPU without camera or display involvement.  

# 1. Introduction 
This document provides usage examples of using the TOPST AI-G.   
...
# 2. Camera Connection
The TOPST AI-G board supports camera input through a MIPI CSI2 interface.
...
As explained above, the TOPST AI-G board provides a 2-lane CSI connection by default, with optional support for a 4-lane configuration through a 20-pin connector for higher data throughput.  

Currently, the TOPST AI-G board supports only the ArduCam (5 MP) and Raspberry Pi v1 Camera (5 MP) modules.
<br/><br/>
...
The TOPST AI-G board supports ArduCam modules with 2-lane or 4-lane MIPI CSI interfaces, enabling stable image input for AI vision tasks such as object detection and image classification. Thanks to its compatibility with FFC cables, the ArduCam module can easily be connected to the TOPST AI-G board's CSI connector, offering a reliable solution for real-time camera input in edge AI systems.
...
| Compatibility            | Raspberry Pi and TOPST AI-G (through MIPI CSI port)      |
...
<p align="center"><strong>Figure 2.1 ArduCam </strong></p><br/>

#### Step 1. Connect ArduCam to TOPST AI-G MIPI CSI
...
<p align="center"><strong>Figure 2.2 Connect ArduCam to TOPST AI-G </strong></p><br/>

#### Step 2. Verifying MIPI CSI Connection on TOPST AI-G Board
To check if the ArduCam is properly connected to the TOPST AI-G board, enter the following command and check.  
```
$ dmesg | grep -i "ov5647"
```
...
Originally designed for the Raspberry Pi series, this module is also compatible with the TOPST AI-G board through the 15-pin MIPI CSI connector. It provides a reliable solution for entry-level camera applications such as image capture, video streaming, and AI-powered computer vision tasks on the AI-G platform.
...
You can test the Rasberry pi v1 camera by following these steps:
...
<p align="center"><strong>Figure 2.3 Raspberry Pi v1 Camera </strong></p><br/>

#### Step 1. Connect Raspberry pi v1 camera to TOPST AI-G MIPI CSI
<p align="center"><img src="https://raw.githubusercontent.com/topst-development/Documentation/refs/heads/main/Assets/TOPST%20AI-G/Available%20Applications/Figure%202.%20Connecting%20Cam%20to%20AI-G.png" width="500"></p>
<p align="center"><strong>Figure 2.4 Connect Raspberry Pi v1 Camera to TOPST AI-G </strong></p><br/>

#### Step 2. Verifying MIPI CSI Connection on TOPST AI-G Board
To check if the Raspberry pi camera is properly connected to the TOPST AI-G board, enter the following command and check.  
```
$ dmesg | grep -i "ov5647"
```
...
# 3. Video Output
The TOPST AI-G board supports video output through a MIPI DSI-2 interface.
...
As mentioned above, the TOPST AI-G board provides a 2-lane DSI connection through a 15-pin FFC connector, allowing connection to compatible MIPI DSI display modules for GUI rendering, video playback, or real-time inference visualization.
<br/><br/>

### 3.1.1 5 Inch DSI Display
The Elecrow 5 Inch DSI Display is a 800×480 resolution LCD with an IPS panel and capacitive touch support, designed for use with the Raspberry Pi’s MIPI DSI interface. Thanks to its plug-and-play functionality, no additional driver installation is required. The display connects to the TOPST AI-G board through the 15-pin MIPI DSI connector and is powered directly through the interface, eliminating the need for an external power supply.  
This module is suitable for GUI-based applications, real-time monitoring, and interactive AI demos on the TOPST AI-G platform.
...
| Cable Type          | FFC (15-pin)                               |
| Compatibility       | Raspberry Pi and TOPST AI-G (through MIPI DSI interface)|
...
<p align="center"><strong>Figure 3.1 5 Inch DSI Display </strong></p><br/>

#### Step 1. Connect Display to TOPST AI-G MIPI DSI
...
<p align="center"><strong>Figure 3.2 Connect Display to TOPST AI-G </strong></p><br/>

#### Step 2. Verify MIPI DSI Connection on TOPST AI-G Board
To check if the display is properly connected to the TOPST AI-G board, enter the following command and check.  
```
...
This chapter covers how to connect the TOPST AI-G board to various storage devices.
Supported storage options include external storage through PCIe.
<br/><br/><br/>


## 4.1 NVME M.2 SSD
The TOPST AI-G board supports M.2 SSD storage devices through its PCIe slot.

#### Step 1. Connect the SSD
- NVMe SSD (M.2 PCIe): Insert the NVMe M.2 SSD into the TOPST AI-G board’s PCIe slot. 
...
<p align="center"><strong>Figure 4.1 Connect NVME M.2 SSD to TOPST AI-G</strong></p><br/>

#### Step 2. Boot the TOPST AI-G Board
...
The TOPST AI-G board supports Ethernet connectivity through its onboard RJ45 Ethernet port. This allows the board to communicate with local networks or the internet by using standard TCP/IP protocols. Ethernet is commonly used for deploying AI applications that require remote access, data streaming, or software updates.
<br/><br/><br/>

## 5.1 Network Connection Through Router
This method connects the AI-G board to a local network by using a standard router. The board can obtain an IP address automatically through DHCP or be configured with a static IP address.
<br/><br/>
...
### 5.1.2 Create the Network Configuration File
...
### 5.1.3 Restart the Network Service
...
## 5.2 Network Sharing with the Host PC
...
4. In the Home networking connection dropdown menu, select the Ethernet adapter that the TOPST AI-G board will connect to (for example, "Ethernet").
...
### 5.2.2 Remount the Root Filesystem as Read/Write
Root filesystem is mounted as read-only. Remount it with write permissions to allow editing configuration files by using the following command:
...
### 5.2.3 Create the Network Configuration File
Create or edit the /etc/systemd/network/20-wired.network file with the following content to enable DHCP on the eth0 interface.
...
### 5.2.4 Restart the Network Service
Apply the new network configuration by restarting the systemd-networkd service:
...
<p align="center"><strong>Figure 5.5 Nework Sharing with Host PC</strong></p><br/>
...
<p align="center"><strong>Figure 6.1 40-pin GPIO Header Pinmap of TOPST AI-G  </strong></p><br/>
...
<p align="center"><strong>Figure 6.2 TOPST AI-G GPIO LED Circuit Schematic  </strong></p><br/>

##### Step 2.1 Pin Mapping
The following table shows pin mapping.

<div style="text-align: center;">
  <p><strong>Table 6.1 Pin Mapping of TOPST AI-G LED</strong></p>
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
      <th>GPIO</th>
    </tr>
    <tr>
...
To operate the LED connected to GPIO72 on the TOPST AI-G board, run the following code:
...
<p align="center"><strong>Figure 6.3 TOPST AI-G GPIO Button Circuit Schematic  </strong></p><br/>
...
<div style="text-align: center;">
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
   <p><strong>Table 6.2 Pin Mapping of TOPST AI-G Button</strong></p>
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
      <th>GPIO</th>
...
#### Step 3. How to execute
To monitor the button input connected to GPIO79 on the TOPST AI-G board, run the following code:
...
To stop the script, press **[Ctrl+C]**.  
When the script is terminated, GPIO79 is automatically unexported and cleand up.
Make sure to give the script execute permission before running:
```bash
...
**Note:** GPIO72 and GPIO79 are used here as examples. You may use any available GPIO pin on the TOPST AI-G board based on the 40-pin header pinmap. Refer to the official pinout diagram and select a GPIO number that suits your hardware configuration.
<br/><br/><br/>

### 6.1.3 Touch Senosr
A touch sensor is a digital input device that detects physical contact and outputs a high signal when touched.
In this example, the touch sensor is connected to the TOPST AI-G board, and its output is monitored through GPIO88.
...
#### Step 2. Example Circuit
- Touch Sensor
    - VCC pin of the Touch Sensor is connected to the 3.3V pin on the TOPST AI-G board.
    - GND pin of the Touch Sensor is connected to GND on the TOPST AI-G board.
    - OUT pin of the Touch Sensor is connected to pin 8 (GPIO88) of the TOPST AI-G board.
...
<p align="center"><strong>Figure 6.4 TOPST AI-G GPIO Touch Sensor Circuit Schematic  </strong></p><br/>
...
<div style="text-align: center;">
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
   <p><strong>Table 6.3 Pin Mapping of TOPST AI-G Touch Sensor</strong></p>
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
      <th>GPIO</th>
...
#### Step 3. How to execute
To monitor the touch sensor input connected to GPIO88 on the TOPST AI-G board, run the following code:
...
**Note**: GPIO88 is used here as an example. You may use any available GPIO pin on the TOPST AI-G board based on the 40-pin header pinmap. Refer to the official 40-pin pinout diagram of the TOPST AI-G board to avoid conflicts.
<br/><br/><br/>
...
In this example, the sensor is connected to the TOPST AI-G board, and vibration input is detected through a digital GPIO 89.
...
- Vibration Detection Sensor
    - VCC pin of the Vibration Detection Sensor is connected to the 5V pin on the TOPST AI-G board.
    - GND pin of the Vibration Detection Sensor is connected to GND on the TOPST AI-G board.
    - Digital Output (DO) pin of the Vibration Detection Sensor is connected to pin 10 (GPIO89) of the TOPST AI-G board.
...
<p align="center"><strong>Figure 6.5 TOPST AI-G GPIO Vibration Detection Sensor Circuit Schematic  </strong></p><br/>
...
<div style="text-align: center;">
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
   <p><strong>Table 6.4 Pin Mapping of TOPST AI-G Vibration Detection Sensor</strong></p>
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
      <th>GPIO</th>
...
#### Step 3. How to execute
To monitor the vibration detection sensor input connected to GPIO89 on the TOPST AI-G board, run the following code:
...
**Note**: GPIO89 is used here as an example. You may use any availble GPIO pin on the TOPST AI-G board based on the 40-pin header pinmap. Refer to the official 40-pin pinout diagram of the TOPST AI-G board to avoid conflicts.
<br/><br/><br/>
...
In this example, the SZH-SSBH-002 infrared sensor is connected to the TOPST AI-G board, and obstacle detection is monitored through digital GPIO 88.
...
#### Step 2. Example Circuit
- Infrared Sensor
    - VCC pin of the Infrared Sensor is connected to the 3.3V pin on the TOPST AI-G board.
    - GND pin of the Infrared Sensor is connected to GND on the TOPST AI-G board.
    - OUT pin of the Intrared Sensor is connected to pin 8 (GPIO88) of the TOPST AI-G board.
...
<p align="center"><strong>Figure 6.6 TOPST AI-G GPIO Infrared Sensor Circuit Schematic  </strong></p><br/>
...
<div style="text-align: center;">
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
   <p><strong>Table 6.5 Pin Mapping of TOPST AI-G Infrared Sensor</strong></p>
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
      <th>GPIO</th>
...
#### Step 3. How to execute
To monitor the infrared sensor input connected to GPIO88 on the TOPST AI-G board, run the following code:
...
In this example, the sensor is connected to the TOPST AI-G board and gas presence is detected through GPIO88.
...
- Air Pollution Detection Sensor
    - VCC pin of the Air Pollution Detection Sensor is connected to the 5V pin on the TOPST AI-G board.
    - GND pin of the Air Pollution Detection Sensor is connected to GND on the TOPST AI-G board.
    - Digital Output (DO) pin of the Air Pollution Detection Sensor is connected to pin 8 (GPIO88) of the TOPST AI-G board.
...
<p align="center"><strong>Figure 6.7 TOPST AI-G GPIO Air Pollution Detection Sensor Circuit Schematic  </strong></p><br/>
...
<div style="text-align: center;">
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
   <p><strong>Table 6.6 Pin Mapping of TOPST AI-G Air Pollution Detection Sensor</strong></p>
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
      <th>GPIO</th>
...
#### Step 3. How to execute
To monitor the air pollution detection sensor input connected to GPIO88 on the TOPST AI-G board, run the following code:
...
**Note**: GPIO88 is used here as an example. You may use any availble GPIO pin on the TOPST AI-G board based on the 40-pin header pinmap. Refer to the official 40-pin pinout diagram of the TOPST AI-G board to avoid conflicts.
<br/><br/><br/>


## 6.2 I2C
The TOPST AI-G board provides I2C communication through the 40-pin GPIO header, allowing it to interface with various peripherals such as sensors, displays, and expansion modules.  
...
On the TOPST AI-G board, the LCD's SDA and SCL lines can be connected to GPIO pins configured for I2C. Once connected, the LCD can be controlled using the Linux I2C tools or custom software.
...
<p align="center"><strong>Figure 6.8 TOPST AI-G I2C LCD Display Circuit Schematic  </strong></p><br/>

##### Step 2.1 Pin Mapping
The following table shows pin mapping.

<div style="text-align: center;">
  <p><strong>Table 6.7 Pin Mapping of TOPST AI-G LCD Display</strong></p>
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
...
Prepare the cross-compilation toolchain for the TOPST AI-G board, then compile the program using the following command:
...
I2C_TEST.c I2C_TEST
```
##### Step 3.3 Transfer and Execute the program on the TOPST AI-G Board
After cross-compiling the binary on your host system, transfer the binary to the TOPST AI-G board by using the following command:

```
$ scp I2C_TEST root@192.168.0.100:/home/root/
```

On the TOPST AI-G board, execute the program by using the following command:
```
$ chmod + x I2C_TEST
$ ./I2C_TEST
```
...
#### Step 4. Execution Result
This program initializes the 1602A I2C LCD Display and prints a test message through the I2C interface on the AI-G board. When executed, the message "test pass!" will appear on the LCD screen.  
...
## 6.3 SPI
The TOPST AI-G board supports Serial Peripheral Interface (SPI) communication through a 40-pin GPIO header, enabling data exchange between external devices and the board.  
...
### 6.3.1 Dot Matrix
8x8 dot matrix display is commonly used for simple text or pattern output in embedded systems. On the TOPST AI-G board, the dot matrix module can be controlled through SPI using a driver chip such as the MAX7219.
...
<p align="center"><strong>Figure 6.9 TOPST AI-G SPI Dot Matrix Circuit Schematic  </strong></p><br/>

##### Step 2.1 Pin Mapping
The following table shows pin mapping.

<div style="text-align: center;">
  <p><strong>Table 6.8 Pin Mapping of TOPST AI-G Dot Matrix</strong></p>
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
    <tr>
      <th colspan="3">Pin Name</th>
      <th>TOPST AI-G Board</th>
...
#### Step 3. How to execute
To communicate with SPI devices on the TOPST AI-G board, you need to write a C program using the spidev interface and cross-compile it for the target architecture.  
...
## 6.4 UART
The TOPST AI-G board supports Universal Asynchronous Receiver/Transmitter (UART) communication through its 40-pin GPIO header, enabling serial communication with devices such as serial consoles and other microcontrollers.  
...
UART loopback test is a simple way to verify that UART communications is functioning correctly on the TOPST AI-G board.  
...
<p align="center"><strong>Figure 6.10 TOPST AI-G UART Circuit Schematic  </strong></p><br/>

##### Step 2.1 Pin Mapping
The following table shows pin mapping.

<div style="text-align: center;">
  <p><strong>Table 6.9 Pin Mapping of TOPST AI-G UART</strong></p>
  <table style="margin: 0 auto; border-collapse: collapse;" border="1">
    <tr>
...
To test UART loopback on the TOPST AI-G board, run the following script after connecting the TX and RX pins together.

...
Before running the `make` command, you must delete the existing post_process.c and network.h in the build_nerwork folder.  
...
Supported layer type specification by AI-G NPU.

  **Table 5.1 Sample Networks Description:**
...
| Depth-wise Convolutio   | Kernel size: 1x1 to 5x5<br/>Stride: 1 to 2<br/>Padding: 0 to 2|
...
| Sub                     | O (Converted to Add opeator)                                 |
...
## 5.4 TSENSOR FLOW LITE OPERATORS
Note: Even if the operator is not supported, the operation of the last layer of the neural network can be processed through the Host 
CPU.

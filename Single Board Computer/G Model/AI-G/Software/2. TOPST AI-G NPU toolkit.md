# 1. Introduction
Neural network Processing Unit (NPU) is an HW neural network accelerator and the NPU in TCC750x is optimized for the inference of neural networks for vision applications, such as object detection, image classification, and face detection.  
...
## 2.1 TOPST AI-G NPU Toolkit Structure
NPU development
...
# 3. Build Guide
This chapter describes the process of converting and building neural network models for execution on the TOPST AI-G NPU. It begins with the use of the Converter tool, which transforms standard models into a format compatible with the NPU.
<br/><br/>
...
model, which is used in AI-G board.
<br/><br/>
...
Before running the make command, you must delete the existing post_process.c and network.h files in the build_nerwork folder.  
...
1. Connect the camera and DSI display and boot the AI-G board.
...
The AI-G NPU supports the folloing layer types.

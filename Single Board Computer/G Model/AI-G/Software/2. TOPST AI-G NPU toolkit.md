...
|-------------------------|-------------------------------------------------------------|
| Max Pool 2D             | Kernel size: 2x2 to 13x13<br/>stride: 1 to 4<br/>padding: 0 to 6 |
| Average Pool 2D         | Kernel size: 2x2 to 13x13<br/>stride: 1 to 4<br/>padding: 0 to 6 |
...
| Activation Function     | Bypass<br/>ReLU, Leaky ReLU<br/>Sigmoid<br/>Tanh<br/>Mish<br/>Swish| 
| Element-wise            | Addition <br/> Scalar + Tensor <br/> Vector + Tensor<br/> Tensor + Tensor <br/>Multiplication <br/> Scalar * Tensor <br/> Vector * Tensor<br/> Tensor * Tensor      | 
| Upscale                 | 2x, Nearest Neighborhood                                    | 
| Global Average Pool     |                                                             |  
| Batch Normalization     |                                                             |  
| Concatenation           |                                                             | 
...
| AveragePool             | kernel: 2x2 to 13x13 <br/>stride: 1 to 13<br/>Padding: 0 to 6|
| BatchNorm               | O                                                           |
| Concat                  | Channel-wise concatenation only (Only 32-aligned channel size) | 
| Constant                | O                                                           |  
| ConstantOfShape         | O                                                           | 
| Conv                    | conv2d<br/>kernel: 1x1 to 7x7<br/>padding: 0 to 3<br/>stride: 1 to 4<br/>dilation: 1 to 4<br/>dw-conv<br/>kernel: 1x1 to 5x5<br/>padding: 0 to 2<br/>stride: 1 to 2<br/>dilation: 1     | 
| Conv Transposed         |  stride=2 only                                              |  
| Div                     | constant divisor only                                       |  
| Flatten                 | O                                                           | 
| Gemm                    | O                                                           |
| GlobalAveragePool       | O                                                           |
| LeakyRelu               | O                                                           | 
| MatMul                  | One tensor should be constant.                              | 
| MaxPool                 | kernel: 2x2 to 13x13<br/>stride: 1 to 13<br/>Padding: 0 to  | 
| Mul                     | Scalar + Tensor,<br/>Tensor + Tensor,<br/>Vector + Tensor   | 
| Pad                     | pad followed by conv layer only<br/>zero padding only<br/>pad size: 0 to 3    | 
| ReduceMean              | axes = [h, w] only                                          |                                                      
| ReduceSum               | axes = [h, w] only                                          |
| Relu                    | O                                                           |  
| Resize                  | 2x nearest-neighborhood upsample only                       | 
| Shape                   | O                                                           |  
| Sigmoid                 | O                                                           | 
| Slice                   | Channel-wise: 32-aligned channel size<BR/>Row-wise: supported  | 
| Sub                     | O                                                           |  
| Tanh                    | O                                                           | 
| Unsqueeze               | O                                                           | 
| Upsample                | 2x nearest-neighborhood upsample only                       | 
...
|         Operator        |                         Dedicated HW                        |
|-------------------------|-------------------------------------------------------------|
| Conv                    | conv2d<br/>kernel: 1x1 to 7x7<br/>padding: 0 to 3<br/>stride: 1 to 4<br/>dilation: 1 to 4 |
| Convolution             | dw-conv<br/>kernel: 1x1 to 5x5<br/>padding: 0 to 2<br/>stride: 1 to 2<br/>dilation: 1|
| max                     | kernel: 2x2 to 13x13<br/>stride: 1 to 13<br/>Padding: 0 to 6|
| maxpool                 | kernel: 2x2 to 13x13<br/>stride: 1 to 13<br/>Padding: 0 to 6| 
| local_avgpool           | size=2, stride=2 only                                       | 
| avg                     | O                                                           | 
| avgpool                 | O                                                           |  
| upsample                | stride=2 only                                               |  
| route                   | direction: channel<br/>32-aligned channel size only         |  
| shortcut                | O                                                           | 
| reorg                   | x                                                           |
| reorg3d                 | O                                                           |
| region                  | Processed thorugh CPU                                       | 
| yolo                    | Processed thorugh CPU                                       |  
| batchnorm               | O                                                           | 
| activation              | relu, relu6, swish, mish, leaky, tanh, logistic             | 
| connected               | O                                                           | 
| deconvolution           | 2x2 only                                                    | 
| sam                     | O                                                           |
...
|         Operator        |                         Dedicated HW                        |
|-------------------------|-------------------------------------------------------------|
| CONV_2D                 | conv2d<br/>kernel: 1x1 to 7x7<br/>padding: 0 to 3<br/>stride: 1 to 4<br/>dilation: 1 to 4 |
| DEPTHWISE_CONV_2D       | dw-conv<br/>kernel: 1x1 to 5x5<br/>padding: 0 to 2<br/>stride: 1 to 2<br/>dilation: 1|
| FULLY_CONNECTED         | O                                                           |
| CONCATENATION           | Channel-wise concatenation only (Only 32-aligned channel size) | 
| ADD                     | Scalar + Tensor<br/>Vector + Tensor<br/>Tensor + Tensor     | 
| SUB                     | O (Converted to add layer)                                  | 
| MUL                     | Scalar * Tensor<br/>Vector * Tensor<br/>Tensor * Tensor     | 
| DIV                     | constant divisor only                                       |  
| AVERAGE_POOL_2D         | Filter: 2 to 13<br/>stride: 1 to 13<br/>pad: 0 to 6         |  
| MAX_POOL_2D             | Filter: 2 to 13<br/>stride: 1 to 13<br/>pad: 0 to 6         | 
| RELU                    | O                                                           |
| RELU6                   | O                                                           |
| HARD_SWISH              | O                                                           | 
| LEAKY_RELU              | O                                                           | 
...
| LOG_SOFTMAX             | X                                                           | 
| MEAN                    | axes = [h, w] only                                          | 
| PAD                     | pad followed by conv layer only<br/>zero padding only<br/>pad size <= 3                                                            | 
| RESIZE_NEAREST_NEIGHBOR | O                                                           |
| TRANSPOSE_CO            | stride: 2x2 only                                            | 
